...
class MainViewModel(private val context: Context) : ViewModel() {
    ...
    private val _filterType    = MutableLiveData<DocumentType?>(null)
                ...
                val found = repository.scanDevice { path, count ->
                    _scanState.postValue(
                        ScanState(
                            ...
                            currentPath = path,
                            foundCount  = count,
                            progress    = minOf(99, count * 4)
                        )
                    )
                }
                val paths = found.map { it.file.absolutePath }.toSet()
                val merged = found.toMutableList()
                internal.filter { it.file.absolutePath !in paths }.forEach { merged.add(it) }
                _allDocuments.value = merged
    ...
    private fun applyFilterSort(): List<DocumentFile> {
        ...
        var list = (_allDocuments.value ?: emptyList()).filter { doc ->
